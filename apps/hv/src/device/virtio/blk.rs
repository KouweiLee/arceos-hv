...
use libax::hv::vm_ipa2pa;
use spin::Mutex;

use crate::device::VIRTQ_DESC_F_WRITE;

use super::{VirtioMmio, Virtq};
...
pub const BLOCKIF_SIZE_MAX: usize = 128 * 4096;
        ...
        loop {
            ...
            if vq.desc_has_next(next_desc_idx) {
                ...
                if head {
                    ...
                    /*data handler*/
                    if (vq.desc_flags(next_desc_idx) & VIRTQ_DESC_F_WRITE) as u32 >> 1
                        == req_node.req_type
                    {
